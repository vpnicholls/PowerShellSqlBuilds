...
$EnvironmentName = "UAT"
$BackupDirectory = "\\Server\FileshareBackups\$EnvironmentName\User\"
# $OutputFileName = '...\JobLogs\BackupUserDatabases.log' # Need to programmatically get error log location
...
exec (''dbcc shrinkfile ('''''' + @DBName + ''_log'''' , 0, truncateonly)'')
'
'@
...
<#
Need to tell SQL when during the day we want to acutally run it. This is a timespan base on 00:00:00 as the start (i.e. midnight).
